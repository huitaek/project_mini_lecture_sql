...
def read_csv_by_pandas(file:str, encoding:str):
    return pd.read_csv(file, encoding=encoding)  # utf-8 or euc-kr

# 날씨 데이터 null 값 전처리 함수
@error_Logging_decorator
def preprocessing_weather_data(data:pd.DataFrame):
    values = []
...
# 날씨 데이터를 DB에 넣는 함수
@error_Logging_decorator
def insert_to_db_weather_data(db,file_names:str):
...
# 사고 데이터를 DB에 넣는 함수
@error_Logging_decorator
def insert_to_db_accident_data(db:DB,file_names:str):
...
# 테이블 이름
@error_Logging_decorator
def make_csv_file_from_table_data(db:DB,tableName:str,query:str='select * from'):
...
@error_Logging_decorator
def insert_from_csv(db:DB, table_name:str, columns:list, dataFrame:pd.DataFrame):
    proceed_values = []
